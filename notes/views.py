...
@login_required
def regenerate_tags(request, pk):
    """AJAX: стереть текущие теги заметки и сгенерировать их заново через ИИ.

    Полезно после правки заметки, когда старые теги могли устареть.
    Если провайдер ИИ не настроен — ничего не трогаем и возвращаем
    понятную ошибку, чтобы не оставить заметку вовсе без тегов.
    """
    ...
    if not settings or not settings.full_model_code:
        return JsonResponse({
            'status': 'error',
            'message': 'Сначала настройте провайдера ИИ в настройках.'
        }, status=400)

    note.tags.clear()
    run_ai_analysis(note)

    tags = list(note.tags.values_list('name', flat=True).order_by('name'))

    if not tags:
        return JsonResponse({
            'status': 'error',
            'message': 'ИИ не смог подобрать теги. Проверьте настройки провайдера или API-ключ.',
            'tags': [],
        }, status=502)
